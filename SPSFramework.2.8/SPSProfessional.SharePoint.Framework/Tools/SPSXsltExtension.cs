// File : SPSXsltExtension.cs
// Date : 26/04/2008
// User : csegura
// Logs
// 26/04/2008 (csegura) - Created.
...
namespace SPSProfessional.SharePoint.Framework.Tools
{
    ...
    {
        private const string LOOKUP_SEPARATOR = ";#";
        ...
        public string UrlParam(string parameter, string value)
        {
        ...
        public string LookupValue(string lookupIdValue)
        {
            int index = lookupIdValue.IndexOf(LOOKUP_SEPARATOR);

            if (index != -1)
            {
                lookupIdValue = lookupIdValue.Remove(0, index + 2);
            }
        ...
        public string LookupId(string lookupIdValue)
        {
            int index = lookupIdValue.IndexOf(LOOKUP_SEPARATOR);

            if (index != -1)
            {
                lookupIdValue = lookupIdValue.Substring(0, index);
            }

            return lookupIdValue;
        }

        public string LookupParam(string lookupIdValue, string param)
        {
            // &Param=1
            param = "&" + param + "=" + LookupId(lookupIdValue);
            return param;
        }

        #endregion

        public string Replace(string source, string search, string text)
        {
            return source.Replace(search, text);
        }

        public string XmlDecode(string source)
        {
            return XmlConvert.DecodeName(source);
        }
        ...
        {
            return DateTime.Parse(theDate, CultureInfo.CurrentCulture); //.ToUniversalTime().ToLocalTime();
        ...
        public string FormatDateTime(string stringDate, string formatString)
        {
            ...
            try
            {
                ...
                Debug.WriteLine("DateTime:"+stringDate+" dateTime:"+dateTime);
        ...
        /// <param name="formatString">The format string.</param>
        /// <returns>The formated date</returns>
        public string FormatIsoDateTime(string stringDate, long lcid, string formatString)
        {
            string str = string.Empty;
            try
            {
                DateTime dateTime = SPUtility.CreateDateTimeFromISO8601DateTimeString(stringDate);
        ...
        /// <param name="formatString">The format string.</param>
        /// <returns>The formated date</returns>
        public string FormatIsoDateTime(string stringDate, string formatString)
        {
            string str = string.Empty;
            int lcid = SPContext.Current.Web.CurrencyLocaleID;
        ...
        public string GetStringBeforeSeparator(string inputString)
        {
            int index = inputString.IndexOf(LOOKUP_SEPARATOR);
            if (index != -1)
            {
