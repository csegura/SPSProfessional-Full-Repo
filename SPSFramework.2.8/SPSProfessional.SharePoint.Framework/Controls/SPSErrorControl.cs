...
namespace SPSProfessional.SharePoint.Framework.Controls
{
    ...
    public class SPSErrorControl : Control, ISPSErrorPool
    {
        #region CONSTS
        ...
        public override void RenderControl(HtmlTextWriter writer)
        {
            if (_errors.Count > 0)
            {
                ...
                foreach (SPSErrorArgs error in _errors)
                {
                    ...
                    Exception ex = error.InternalException;
                    ...
                    if (!CustomError(userMessage, writer))
                    {
                        ...
                        //stringBuilder.Append(ex.Message + "<br/>");

                        if (_showExtendedErrors)
                        {                            
                            while (ex != null)
                            {
                                stringBuilder.Append("<b>" +
                                                     ex.GetType().Name + "</b>(" + ex.Source + ")<br/>" +
                                                     ex.TargetSite + "<br/>" +
                                                     ex.Message + "<br/>");
                        ...
                        if (error.InternalException != null)
                        {
                            writer.WriteLine(SPSErrorBoxEntry,
                                             error.InternalException.Source,
                                             error.InternalException.Message,
                                             stringBuilder);
                        }
                    }
                }
