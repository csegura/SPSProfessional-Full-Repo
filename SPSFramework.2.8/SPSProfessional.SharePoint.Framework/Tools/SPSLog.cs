...
namespace SPSProfessional.SharePoint.Framework.Tools
{
    public static class SPSLog
    {
        ...
        public static void Log(string message, TraceSeverity severity)
        {
        ...
        private static UInt64 _hTraceLog;

        private static UInt64 _hTraceReg;
        ...
        internal static class NativeMethods
        {
            ...
            internal struct ULSTraceHeader
            {
                ...
                [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 32)] internal string wzExeName;

                [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 32)] internal string wzProduct;

                [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 32)] internal string wzCategory;

                [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 800)] internal string wzMessage;
            }
            ...
            internal unsafe delegate uint EtwProc(
                WMIDPREQUESTCODE requestCode, IntPtr requestContext, uint* bufferSize, IntPtr buffer);


            // Copied from Win32 APIs

            [DllImport("advapi32.dll", CharSet = CharSet.Unicode)]
            internal static extern unsafe uint RegisterTraceGuids([In] EtwProc cbFunc, [In] void* context,
                                                                  [In] ref Guid controlGuid, [In] uint guidCount,
                                                                  IntPtr guidReg, [In] string mofImagePath,
        ...
        public static unsafe void RegisterTraceProvider()
        {
            ...
            uint result = NativeMethods.RegisterTraceGuids(ControlCallback, null, ref traceGuid, 0, IntPtr.Zero, null,
                                                           null, out _hTraceReg);
        ...
        {
            uint result = NativeMethods.UnregisterTraceGuids(_hTraceReg);
        ...
        internal static unsafe uint ControlCallback(NativeMethods.WMIDPREQUESTCODE RequestCode, IntPtr Context,
                                                    uint* InOutBufferSize, IntPtr Buffer)
        {
            uint Status;
            ...
            switch (RequestCode)
            {
                case NativeMethods.WMIDPREQUESTCODE.WMI_ENABLE_EVENTS:
                    ...
                    _hTraceLog = NativeMethods.GetTraceLoggerHandle(Buffer);
