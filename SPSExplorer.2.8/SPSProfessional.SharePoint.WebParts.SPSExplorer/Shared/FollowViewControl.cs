    ...
    internal class FollowViewControl : SPControl, INamingContainer
    {
        ...
        protected string CurrentFolder
        {
            ...
            {                
                ...
                {
                    Debug.WriteLine(string.Format("-> FollowView Get {0} => {1}", _listViewGuid, GetList().RootFolder.ServerRelativeUrl));

                    // return list root folder
                    return GetList().RootFolder.ServerRelativeUrl;
                }

                Debug.WriteLine(string.Format("-> FollowView Get {0} => {1}", _listViewGuid, ViewState[CurrentFolderParameter].ToString()));
            ...
            set
            {
                Debug.WriteLine(string.Format("-> FollowView Set {0} => {1}", _listViewGuid, value));
                ViewState[CurrentFolderParameter] = value;
            }
        }
        ...
        {
            Debug.WriteLine("+ GetCurrentFolder");
            GetFolderFromQueryString();
            Debug.WriteLine("+ GetCurrentFolder " + CurrentFolder);
        ...
        protected SPFolder GetFolder()
        {
            if (_folder == null)
            {
               Debug.WriteLine("GetFolder() => " + CurrentFolder);
