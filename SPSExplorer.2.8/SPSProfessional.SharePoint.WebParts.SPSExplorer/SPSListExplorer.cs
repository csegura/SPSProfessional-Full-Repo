...
namespace SPSProfessional.SharePoint.WebParts.SPSExplorer
{
    ...
    public class SPSListExplorer : SPSWebPart, IWebPartCache
    {
        ...
        {
            if (CheckViewToolBar(viewToolBar))
            {
        ...
        private static bool CheckViewToolBar(Control viewToolBar)
        {
            if (viewToolBar.Controls.Count > 0)
                if (viewToolBar.Controls[0].Controls.Count > 1)
                    if (viewToolBar.Controls[0].Controls[1].Controls.Count > 1)
                        return true;
            return false;
        }
    }
